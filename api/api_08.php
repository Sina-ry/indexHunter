...
$sql1 = " SELECT * FROM tc_board where b_gubun = '01' and idx > '".$s_idx."' order by idx  limit 0, 1  "	;
...
$sql1 = " SELECT * FROM tc_board where b_gubun = '01' and idx < '".$s_idx."' order by idx desc limit 0, 1  "	;
...
array_push($data, array('b_title'=>$row["b_title"],'b_custname'=>$row["b_custname"], 'b_custname'=>$row["b_custname"], 'b_readcnt'=>$row["b_readcnt"],  'b_regdate'=>format_sdate($row["b_regdate"]),'b_memo'=>str_replace(chr(13),"<br>",$row['b_memo']) , 'b_file01'=>$b_file01 , 'pre'=>$pre, 'next'=>$next ));
